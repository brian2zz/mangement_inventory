...
export default function SupplierDetailPage() {
  ...
  React.useEffect(() => {
    const fetchSupplier = async () => {
      try {
        const res = await apiFetch(`/suppliers/${params.id}`, { method: "GET" })
        if (!res.ok) return
  ...
  const handleSave = async () => {
    setIsLoading(true)
    try {
      const res = await apiFetch(`/suppliers/${params.id}`, {
        method: "PUT",
        body: JSON.stringify({
  ...
  const handleDelete = async () => {
    setIsLoading(true)
    try {
      const res = await apiFetch(`/suppliers/${params.id}`, { method: "DELETE" })
